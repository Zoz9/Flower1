...
public class CalendarActivity extends ActionBarActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        events.add(new Date());
        //bd data
        bdSupport = new BDSupport(this,"flowersstandartdb",1);
        ...
        Cursor c = db.query("flowersstandartdb", null, null, null, null, null, null);
        fAdapterList = new FAdapterList(this,myOjctFlower);

        if (c.moveToFirst()){
            int idColIndex = c.getColumnIndex("id");
            int nameColIndex = c.getColumnIndex("nameFlower");
            Log.d("TAGN", "rc.getString(nameColIndex) = " + nameColIndex);

            do{

                try {
                    Log.d("TAGN", "c.getString(nameColIndex)) = " + c.getString(nameColIndex));
                    //Log.d("TAGN", "sdf.parse(c.getString(nameColIndex)) = " + (Date)df.parse(c.getString(nameColIndex)));
                    String[] p = c.getString(nameColIndex).split(" ");
                    dt = df.parse(c.getString(nameColIndex));//p[0]+" "+p[1]+" "+p[2]+" "+p[5]);
                    myOjctFlower.add(new CatalogFlower(p[0]+" "+p[1]+" "+p[2]+" "+p[5], c.getInt(idColIndex), "flowersstandartdb"));
                    events.add(dt);
                } catch (ParseException e) {
                    e.printStackTrace();
                }
